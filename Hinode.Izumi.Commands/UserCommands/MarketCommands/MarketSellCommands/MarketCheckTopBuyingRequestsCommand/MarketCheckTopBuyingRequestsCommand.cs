...
namespace Hinode.Izumi.Commands.UserCommands.MarketCommands.MarketSellCommands.MarketCheckTopBuyingRequestsCommand
{
    [InjectableService]
    public class MarketCheckTopBuyingRequestsCommand : IMarketCheckTopBuyingRequestsCommand
    {
        ...
        public MarketCheckTopBuyingRequestsCommand(IMediator mediator, ILocalizationService local)
        {
            _mediator = mediator;
            _local = local;
        }
        ...
        public async Task Execute(SocketCommandContext context, MarketCategory category, string pattern = null)
        {
            ...
            var requests = await _mediator.Send(new GetMarketRequestByParamsQuery(category, false, pattern));
