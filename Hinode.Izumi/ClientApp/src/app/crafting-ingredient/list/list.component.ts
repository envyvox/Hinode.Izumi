import { Component, OnInit } from '@angular/core';
import {
  CraftingIngredientService,
  CraftingIngredientWebModel,
  CraftingService,
  CraftingWebModel,
  IngredientCategory
} from '../../shared/web.api.service';
import { MenuItem, MessageService, SelectItem } from 'primeng/api';
import { Router } from '@angular/router';
import { EnumEx } from '../../shared/enum.extensions';

@Component({
  selector: 'app-list',
  templateUrl: './list.component.html',
  styleUrls: ['./list.component.css']
})
export class ListComponent implements OnInit {

  ingredientCategory = IngredientCategory;
  ingredientCategories: SelectItem[];
  craftings: CraftingWebModel[];
  data: CraftingIngredientWebModel[];
  selectedItem: CraftingIngredientWebModel;
  menuItems: MenuItem[];

  constructor(private _router: Router,
              private _messageService: MessageService,
              private _craftingIngredientService: CraftingIngredientService,
              private _craftingService: CraftingService) {
    this.ingredientCategories = EnumEx.getNamesAndValues(IngredientCategory);
    this._craftingService
        .list()
        .subscribe(x => {
          this.craftings = x;
        });
  }

  ngOnInit(): void {
    this.refresh();

    this.menuItems = [
      {
        label: 'Edit',
        icon: 'pi pi-pencil',
        command: () => this.edit(this.selectedItem)
      },
      {
        label: 'Remove',
        icon: 'pi pi-trash',
        command: () => this.remove(this.selectedItem)
      }
    ];
  }

  refresh() {
    this._craftingIngredientService
        .listAll()
        .subscribe(x => {
          this.data = x;
        });
  }

  edit( selectedItem: CraftingIngredientWebModel ) {
    this._router.navigateByUrl(`/crafting-ingredient/edit/${selectedItem.id}`);
  }

  remove( selectedItem: CraftingIngredientWebModel ) {
    this._craftingIngredientService
        .remove(selectedItem.id)
        .subscribe(() => {
          this.refresh();
        });
    this._messageService.add({severity: 'success', key: 'success', summary: 'Success', detail: 'Crafting ingredient deleted!'});
  }

}
